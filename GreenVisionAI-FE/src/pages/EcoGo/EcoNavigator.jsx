...
const EcoNavigator = () => {
  ...
  const handleMapTap = async (evt) => {
    ...
    await checkTrafficClustersAndRecalculate();
  };

  // Check for traffic clusters and recalculate route
  const checkTrafficClustersAndRecalculate = async () => {
  ...
  // Calculate route
  const calculateRoute = async (trafficCluster = null, ecoMode = false) => {
    ...
    try {
      ...
      if (data.routes && data.routes.length > 0) {
        ...
        mapInstanceRef.current.addObjects([currentRouteRef.current, startMarker, endMarker]);
        mapInstanceRef.current.getViewModel().setLookAtData({ bounds: currentRouteRef.current.getBoundingBox() });
  ...
  const isRouteAvoidingTraffic = (polyline, cluster) => {
