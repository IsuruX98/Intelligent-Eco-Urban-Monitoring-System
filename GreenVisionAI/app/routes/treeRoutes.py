...
# Create a Blueprint for tree-related routes
tree_bp = Blueprint('tree_bp', __name__)

# Define the paths for model and encoder
model_tree = tf.keras.models.load_model('C:/Users/Chamith/Desktop/RP_final/Intelligent-Eco-Urban-Monitoring-System/GreenVisionAI/app/artifacts/model_tree.h5')
model_forecast = tf.keras.models.load_model('C:/Users/Chamith/Desktop/RP_final/Intelligent-Eco-Urban-Monitoring-System/GreenVisionAI/app/artifacts/model_forecast.h5')
...
# Compile the CO2 emission prediction model
...
def calc_vegitation_density(image):
    ...
    ndvi = (green_channel - red_channel) / (green_channel + red_channel + 1e-10)
    non_zer_idxs = (image != [0, 0, 0]).all(axis=-1)
    total_pixels = np.sum(non_zer_idxs)
    high_density_percentage = np.sum(np.logical_and((green_channel > 171), non_zer_idxs)) / total_pixels * 100
    medium_density_percentage = np.sum(np.logical_and((green_channel < 171), (green_channel > 86), non_zer_idxs)) / total_pixels * 100
    low_density_percentage = np.sum(np.logical_and((green_channel < 86), (green_channel > 0), non_zer_idxs)) / total_pixels * 100
    ...
    high_density_percentage = f"{100 - low_density_percentage - medium_density_percentage:.2f} %"
    return image, {
...
def haversine_distance(p1, p2, R=6373.0):
    lon1, lat1 = p1
    lon2, lat2 = p2
    lat1 = radians(lat1)
    lon1 = radians(lon1)
...
def inference_forecast(location_id, selective_window=12, data_path="data/TreeSegmentation/satellite_tree_coverage.csv"):
    ...
    vals = (round(Y_pred[0][location_id], 2), round(Y_pred[1][location_id], 2), round(Y_pred[2][location_id], 2))
    vals = {"month 1": f"{vals[0]} %", "month 2": f"{vals[1]} %", "month 3": f"{vals[2]} %"}
    return vals

# Function to visualize inference results
def visualize_inference_modified(image_path, location, background_color=(0, 0, 0), output_shape_new=(65536, 2), building_color=(255, 255, 255), output_shape_orig=(256, 256, 3)):
    ...
    df_coor['distance'] = df_coor[['Longitude', 'Latitude']].apply(lambda x: haversine_distance((x['Longitude'], x['Latitude']), location), axis=1)
    df_coor = df_coor.sort_values(by='distance')
    location_id = df_coor.index[0]
    print('Location ID : ', location_id)
    green_evolution = inference_forecast(location_id)
    return {
...
def tree():
    ...
    file.save(file_path)
    response = visualize_inference_modified(file_path, location)
    os.remove(file_path)
    return jsonify(response)

@tree_bp.route('/store/<path:filename>')
